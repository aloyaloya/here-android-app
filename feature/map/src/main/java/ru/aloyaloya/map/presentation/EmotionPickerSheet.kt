...
import ru.aloyaloya.map.model.EmotionOption
import ru.aloyaloya.map.model.color
 ...
 *
 * Кнопка «Дальше» активна только после выбора эмоции. Дальше будет экран нового
 * места, пока лист просто закрывается.
 *
...
fun EmotionPickerSheet(
    ...
    onEmotionConfirmed: (EmotionOption) -> Unit,
    modifier: Modifier = Modifier
) {
    var selectedEmotion by rememberSaveable { mutableStateOf<EmotionOption?>(null) }
...
private fun EmotionGrid(
    ...
    onEmotionClick: (EmotionOption) -> Unit
) {
    val spacing = HereSize.EmotionTile.spacing

    Column(verticalArrangement = Arrangement.spacedBy(spacing)) {
        EmotionOption.entries.chunked(EMOTIONS_PER_ROW).forEach { rowEmotions ->
            Row(
